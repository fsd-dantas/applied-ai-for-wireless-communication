...
**A estatística agregada do A\* existe, mas nenhum comando do repositório a recalcula.** A topologia de 17 nós foi removida na v0.10.0, e os números atuais (25,9% no `simulated`, 8,9% no `dual`) foram medidos somando os nós expandidos sobre todos os pares ordenados dos dois cenários. Nenhum teste ou script versionado refaz essa soma: `test_astar_never_expands_more_nodes_than_uniform_cost` verifica a desigualdade, não o percentual. Até que exista esse script, os dois números são um resultado **declarado, não reproduzível por comando** — ver [`research/roadmap.md`](../../research/roadmap.md).

**The aggregate A\* statistic exists, but no command in the repository recomputes it.** The 17-node topology was removed in v0.10.0, and the current figures (25.9% on `simulated`, 8.9% on `dual`) were measured by summing expansions over every ordered pair of both scenarios. No versioned test or script redoes that sum: `test_astar_never_expands_more_nodes_than_uniform_cost` checks the inequality, not the percentage. Until such a script exists, both figures are a **declared result, not reproducible by a command** — see [`research/roadmap.md`](../../research/roadmap.md).
...
**Reproduzível.** `python -m pytest` (ou `make check`) executa os 252 testes; o workflow [`tests.yml`](../../.github/workflows/tests.yml) roda a mesma suíte em CI a cada push/PR na `main`. Cada diagnóstico simulado declara `INDUCIBLE_BY`, o que torna o conjunto de casos reproduzível por terceiros sem acesso a nenhum laboratório. O único item não reproduzível **por comando** hoje é a estatística agregada de ganho do A\*, discutida em [Limitações](#limitações--limitations).

**Reproducible.** `python -m pytest` (or `make check`) runs the 252 tests; the [`tests.yml`](../../.github/workflows/tests.yml) workflow runs the same suite in CI on every push/PR to `main`. Every simulated diagnosis declares `INDUCIBLE_BY`, which makes the case set reproducible by third parties with no access to any laboratory. The one item not reproducible **by a command** today is the aggregate A\* gain statistic, discussed in [Limitations](#limitações--limitations).
